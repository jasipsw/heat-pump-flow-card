...
`;return console.info(`%c  HEAT-PUMP-FLOW-CARD  \n%c  Version ${ut}  \n%c  Built: ${gt}  `,"color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray","color: #95a5a6; font-weight: normal; background: dimgray"),t.HeatPumpFlowCard=class extends lt{constructor(){super(...arguments),this.temperatureHistory=new Map,this.lastRenderTime=0,this.lastHassState={}}static getConfigElement(){}static getStubConfig(){return{type:"custom:heat-pump-flow-card",title:"Heat Pump Flow"}}setConfig(t){if(!t)throw new Error("Invalid configuration");const{animation:e,temperature:i,display:o,heat_pump_visual:r,labels:a,temperature_status:n,...s}=t;this.config={...s,animation:{enabled:!0,min_flow_rate:5,max_flow_rate:1,max_flow_rate_value:50,idle_threshold:0,dot_size:1.5,dot_spacing:30,use_temp_color:!1,dot_color:"rgba(255, 255, 255, 0.75)",dot_opacity:1,...e},temperature:{delta_threshold:10,hot_color:"#e74c3c",cold_color:"#3498db",neutral_color:"#95a5a6",unit:"C",...i},display:{show_values:!0,show_labels:!0,show_icons:!0,compact:!1,decimal_places:1,...o},heat_pump_visual:{off_color:"#95a5a6",heating_color:"#e74c3c",cooling_color:"#3498db",dhw_color:"#e67e22",defrost_color:"#f1c40f",show_metrics:!0,animate_fan:!0,...r},labels:{hp_supply:"HP Supply",hp_return:"HP Return",hvac_supply:"HVAC Supply",hvac_return:"HVAC Return",buffer_tank:"BUFFER",dhw_tank:"DHW",power_in:"Power In",thermal_out:"Thermal Out",cop:"COP",flow:"Flow",energy:"Energy",cost:"Cost",...a},temperature_status:{enabled:!1,change_threshold:.5,time_interval:60,circle_radius:12,increasing_color:"#e74c3c",decreasing_color:"#3498db",steady_color:"#2c3e50",show_caret:!0,points:{hp_outlet:{enabled:!0},hp_inlet:{enabled:!0},buffer_supply:{enabled:!0},buffer_return:{enabled:!0},hvac_supply:{enabled:!0},hvac_return:{enabled:!0},dhw_inlet:{enabled:!0},dhw_outlet:{enabled:!0},...n?.points},...n}}}shouldUpdate(t){if(t.has("config"))return!0;if(t.has("hass")){const t=Date.now();if(t-this.lastRenderTime<1e3)return this.updateAnimationVariables(),!1;this.lastRenderTime=t}return super.shouldUpdate(t)}updated(t){super.updated(t),t.has("hass")&&this.hass&&this.updateAnimationVariables()}firstUpdated(){this.config.animation.enabled&&setTimeout(()=>{this.updateAnimationVariables()},100)}updateAnimationVariables(){this.updateFanAnimation()}updateFanAnimation(){const t=this.shadowRoot?.querySelector("#fan-blades");if(!t||!this.config.heat_pump_visual?.animate_fan)return;const e=this.getHeatPumpState().fanSpeed||0;if(e>0){t.classList.add("fan-rotating");const i=e>0?100/e:999;t.style.setProperty("--fan-duration",`${i}s`)}else t.classList.remove("fan-rotating")}getHeatPumpState(){const t=this.config.heat_pump||{};return{power:this.getStateValue(t.power_entity)||0,thermal:this.getStateValue(t.thermal_entity)||0,cop:this.getStateValue(t.cop_entity)||0,outletTemp:this.getStateValue(t.outlet_temp_entity)||0,inletTemp:this.getStateValue(t.inlet_temp_entity)||0,flowRate:this.getStateValue(t.flow_rate_entity)||0,fanSpeed:this.getStateValue(t.fan_speed_entity),mode:this.getStateString(t.mode_entity),modeDisplay:this.getStateString(t.mode_display_entity),defrost:"on"===this.getStateString(t.defrost_entity),error:this.getStateString(t.error_entity),energy:this.getStateValue(t.energy_entity),cost:this.getStateValue(t.cost_entity),runtime:this.getStateValue(t.runtime_entity)}}getStateString(t){if(!t||!this.hass)return;const e=this.hass.states[t];return e?.state}getBufferTankState(){const t=this.config.buffer_tank||{};return{supplyTemp:this.getStateValue(t.supply_temp_entity)||0,returnTemp:this.getStateValue(t.return_temp_entity)||0,level:this.getStateValue(t.level_entity)}}getHVACState(){const t=this.config.hvac||{};return{thermal:this.getStateValue(t.thermal_entity)||0,flowRate:this.getStateValue(t.flow_rate_entity)||0,supplyTemp:this.getStateValue(t.supply_temp_entity)||0,returnTemp:this.getStateValue(t.return_temp_entity)||0}}getDHWTankState(){const t=this.config.dhw_tank||{};return{inletTemp:this.getStateValue(t.inlet_temp_entity)||0,outletTemp:this.getStateValue(t.outlet_temp_entity)||0,tankTemp:this.getStateValue(t.tank_temp_entity)}}getG2ValveState(){const t=this.config.g2_valve||{},e=this.getStateString(t.state_entity);return{isActive:"on"===e||"true"===e||"1"===e}}getAuxHeaterState(){const t=this.config.aux_heater||{},e=t.enabled||!1,i=this.getStateValue(t.power_entity)||0,o=t.max_power||18e3;return{enabled:e,power:i,maxPower:o,intensity:Math.min(i/o,1),displayName:t.display_name||t.name||"AUX"}}getStateValue(t){if(!t||!this.hass)return;const e=this.hass.states[t];if(!e)return;const i=parseFloat(e.state);return isNaN(i)?void 0:i}getStateUnit(t){if(!t||!this.hass)return"";const e=this.hass.states[t];return e?.attributes?.unit_of_measurement||""}formatValue(t,e=1){return void 0===t?"N/A":t.toFixed(e)}trackTemperature(t,e){if(!t)return;const i=Date.now(),o=this.temperatureHistory.get(t)||[];o.push({temperature:e,timestamp:i});const r=i-2*(1e3*(this.config.temperature_status?.time_interval||60)),a=o.filter(t=>t.timestamp>r);this.temperatureHistory.set(t,a)}getTemperatureTrend(t){if(!t)return"steady";const e=this.temperatureHistory.get(t);if(!e||e.length<2)return"steady";const i=Date.now(),o=1e3*(this.config.temperature_status?.time_interval||60),r=this.config.temperature_status?.change_threshold||.5,a=e[e.length-1],n=i-o,s=e.find(t=>t.timestamp>=n);if(!s||s===a)return"steady";const l=a.temperature-s.temperature;return l>r?"increasing":l<-r?"decreasing":"steady"}getTemperatureStatus(t,e){if(!t||!this.config.temperature_status?.enabled)return;this.trackTemperature(t,e);return{current:e,trend:this.getTemperatureTrend(t),entity:t}}handleTemperatureClick(t,e){if(t.stopPropagation(),!e||!this.hass)return;const i=new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:e}});this.dispatchEvent(i)}renderTemperatureIndicator(t,e,i,o,r){if(!this.config.temperature_status?.enabled||!r?.enabled||!i)return Q``;const a=this.getTemperatureStatus(i,o);if(!a||void 0===a.current)return Q``;const n=this.config.temperature_status,s=n.circle_radius||12,l=!1!==n.show_caret;let h;switch(a.trend){case"increasing":h=n.increasing_color||"#e74c3c";break;case"decreasing":h=n.decreasing_color||"#3498db";break;default:h=n.steady_color||"#2c3e50"}return Q`
      <g class="temp-status-indicator"
        ...
        ${l&&"increasing"===a.trend?Q`
          <path
            d="M ${t-3} ${e-6+3} L ${t} ${e-6} L ${t+3} ${e-6+3} Z"
            fill="${h}" />
        `:""}
        ...
        <div class="card-content">
          <svg viewBox="0 0 800 700" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="260" y="170" text-anchor="middle" fill="${l}"
              ...
              ${this.config.text_style?.show_labels?`${this.config.labels.hp_supply}: `:""}${this.formatValue(t.outletTemp,1)}°${this.getStateUnit(this.config.heat_pump?.outlet_temp_entity)||"C"}
            </text>
            ...
            <text x="260" y="240" text-anchor="middle" fill="${h}"
                  ...
                  font-family="${this.config.text_style?.font_family||"Courier New, monospace"}"
                  font-weight="${this.config.text_style?.font_weight||"bold"}">
              ${this.config.text_style?.show_labels?`${this.config.labels.hp_return}: `:""}${this.formatValue(t.inletTemp,1)}°${this.getStateUnit(this.config.heat_pump?.inlet_temp_entity)||"C"}
            </text>
            ...
            <text x="550" y="170" text-anchor="middle" fill="${c}"
                  ...
                  font-family="${this.config.text_style?.font_family||"Courier New, monospace"}"
                  font-weight="${this.config.text_style?.font_weight||"bold"}">
              ${this.config.text_style?.show_labels?`${this.config.labels.hvac_supply}: `:""}${this.formatValue(e.supplyTemp,1)}°${this.getStateUnit(this.config.buffer_tank?.supply_temp_entity)||"C"}
            </text>
            ...
            <text x="550" y="240" text-anchor="middle" fill="${d}"
                  ...
                  font-family="${this.config.text_style?.font_family||"Courier New, monospace"}"
                  font-weight="${this.config.text_style?.font_weight||"bold"}">
              ${this.config.text_style?.show_labels?`${this.config.labels.hvac_return}: `:""}${this.formatValue(i.returnTemp,1)}°${this.getStateUnit(this.config.hvac?.return_temp_entity)||"C"}
            </text>
            ...
            ${this.renderTemperatureIndicator(400,370,this.config.temperature_status?.points?.dhw_inlet?.entity||this.config.dhw_tank?.inlet_temp_entity,o.inletTemp,this.config.temperature_status?.points?.dhw_inlet)}

            <!-- DHW Tank Outlet (in tank coordinates, bottom of coil at y=470 global) -->
            ${this.renderTemperatureIndicator(400,470,this.config.temperature_status?.points?.dhw_outlet?.entity||this.config.dhw_tank?.outlet_temp_entity,o.outletTemp,this.config.temperature_status?.points?.dhw_outlet)}
          </svg>
        </div>
      </ha-card>
